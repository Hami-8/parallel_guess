...
#ifdef USE_CUDA
    if (pt.content.size()==1){      // ---------- 单 segment ----------
        segment *a = (pt.content[0].type==1) ? &m.letters[m.FindLetter(pt.content[0])]
        ...
        segment* last = (pt.content.back().type==1)? &m.letters[m.FindLetter(pt.content.back())]
